...
import { planningActionsFor, type PlanningAction, type PlanningItem } from '../planning';
...
export function PlanningBoard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await listPlanning(apiFetch, params);
      if (res.status === 401) {
  ...
  const loadCalendar = useCallback(async () => {
    ...
    try {
      ...
      const res = await getPlanningCalendar(apiFetch, from, to);
      if (!res.ok) return;
  ...
  async function runAction(item: PlanningItem, action: PlanningAction, assigneeId?: string) {
    setBusy(true);
    try {
      const res = await transitionPlanning(
        apiFetch,
        item.id,
        action,
        action === 'assign' ? { assigneeId } : undefined,
      );
      if (!res.ok) {
  ...
  async function remove() {
    ...
    setBusy(true);
    try {
      const res = await removePlanning(apiFetch, deleting.id);
      if (!res.ok) {
