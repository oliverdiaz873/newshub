...
import type { ArticleBulkResult, ArticleFilterOption, ArticleListItem } from '../types';
...
export function ArticleList() {
  ...
  const [categories, setCategories] = useState<ArticleFilterOption[]>([]);
  const [authors, setAuthors] = useState<ArticleFilterOption[]>([]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const res = await listArticles(apiFetch, params);
        if (cancelled) return;
        if (res.status === 401) {
  ...
  function summarize(action: BulkAction, results: ArticleBulkResult[]) {
  ...
  async function runBulk(action: BulkAction, ids: string[]) {
    ...
    setBulkBusy(true);
    try {
      const res = await bulkArticles(apiFetch, action, ids);
      if (!res.ok) {
        notify(`HTTP ${res.status}`, 'err');
        return;
      }
      const json = (await res.json()) as { results: ArticleBulkResult[] };
