...
export function CategoryManager() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listCategories(apiFetch, editorialLocale);
      if (res.status === 401) {
  ...
  function startEdit(row: CategoryRow) {
    ...
    setFieldErrors({});
    setTouched(false);
    setForm({ ...EMPTY_CATEGORY_FORM, sort: String(row.sort), esSlug: row.slug, esLabel: row.label, esDesc: row.description ?? '' });
  }
