...
import { notificationHref, typeLabelKey, type NotificationItem } from '../notifications';
import {
  listNotifications,
...
export function NotificationsBoard() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listNotifications(apiFetch, page);
      if (res.status === 401) {
  ...
  async function markOne(id: string) {
    setBusyId(id);
    try {
      const res = await markNotificationRead(apiFetch, id);
      if (!res.ok) {
  ...
  async function markAll() {
    setBusyAll(true);
    try {
      const res = await markAllNotificationsRead(apiFetch);
      if (!res.ok) {
        notify(tc('httpError', { status: res.status }), 'err');
        return;
      }
