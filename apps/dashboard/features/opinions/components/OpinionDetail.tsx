...
import { OpinionEditor } from '@/features/opinions/components/OpinionEditor';
import { getOpinion } from '../services/opinionService';
import type { OpinionFormValue } from '../validate';
import type { EditorialOpinionRead } from '../types';

function toForm(row: EditorialOpinionRead): OpinionFormValue {
...
export function OpinionDetail({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getOpinion(apiFetch, id);
      if (res.status === 401) {
        setError(tc('sessionRequired'));
        return;
      }
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const row = (await res.json()) as EditorialOpinionRead;
      setForm(toForm(row));
