...
function SyndicationBody() {
  ...
  const loadDeliveries = useCallback(
    async (id: string) => {
      ...
      try {
        const res = await apiFetch(`/webhooks/${id}/deliveries?limit=20`);
        if (!res.ok) return;
  ...
  async function toggleActive(item: Subscription) {
    setBusy(true);
    try {
      const res = await apiFetch(`/webhooks/${item.id}`, {
        method: 'PATCH',
  ...
  async function rotate(item: Subscription) {
    setBusy(true);
    try {
      const res = await apiFetch(`/webhooks/${item.id}/rotate`, { method: 'POST' });
      if (!res.ok) {
        notify(`HTTP ${res.status}`, 'err');
        return;
      }
  ...
  async function ping(item: Subscription) {
    ...
    setPingResult(null);
    try {
      const res = await apiFetch(`/webhooks/${item.id}/ping`, { method: 'POST' });
  ...
  async function remove() {
    ...
    setBusy(true);
    try {
      const res = await apiFetch(`/webhooks/${deleting.id}`, { method: 'DELETE' });
      if (!res.ok) {
        notify(`HTTP ${res.status}`, 'err');
        return;
      }
