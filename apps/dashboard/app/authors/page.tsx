...
const EMPTY: AuthorForm = { slug: '', esName: '', esBio: '', enName: '', enBio: '' };
const PAGE_SIZES = [10, 20, 50];

type FormError = Partial<Record<'slug' | 'esName' | 'enName', 'required' | 'slug'>>;

function nameOf(row: AuthorRow, locale: 'es' | 'en' = 'es'): string {
  return row.translations.find((tr) => tr.locale === locale)?.name ?? row.slug;
}

function validateForm(form: AuthorForm): FormError {
...
function AuthorsBody() {
  ...
  async function saveEdit(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await apiFetch(`/authors/${editing}`, { method: 'PATCH', body: JSON.stringify(payload()) });
      if (!res.ok) {
        const body = (await res.json().catch(() => null)) as { code?: string } | null;
        if (body?.code === 'slug_taken') {
          setFieldErrors({ slug: 'slug' });
